...
def processMailbox(config, section):
    ...
    IMAPFOLDER_ORIG = [ folder.strip() for folder in \
                         config.get(section, 'imap_folder').split(',') \
                         if folder.strip() != "" ]
    ...
    ssl = False
    try: 
        ssl_value = config.get(section, 'ssl')
    ...
    try:
        incremental_value = config.get(section, 'incremental_backup')
    ...
    offline = False
    try:
        offline_value = config.get(section, 'offline')
